...
class One_percent_10min(threading.Thread):
    ALGO = "onepercent_10min"
    DATAROAD = './database/data_one_10min.csv'
    ...
    def run(self):
        ...
        while True:
            ...
            if (Manager.THREADING and self._run) is True:
                ...
                # 일단 5개
                money = money_alloc / 5
                self.algo_onepercent_10min(money)
    ...
    def initializer(self):
        ...
        # 데이터 로드
        print("전일 데이터 로드")
        order_data = load_data(self.manager, One_percent_10min.DATAROAD)
        print(order_data)
        # 매도 성사 데이터 삭제
        for_cancel = []
        for i in range(len(order_data)):
            ...
            if ((data['status'] == 'NEW') or (data['status'] == 'wait')):
                ...
                del_data(data, One_percent_10min.DATAROAD)
            ...
            for i in range(len(for_cancel)):
                ...
                try:
                    ...
                    try:
                        ...
                        del_data(req, One_percent_10min.DATAROAD)
                        print("DB 데이터 삭제")
        ...
        msg = {
            "EX": ex,
            "Time": on_time,
            "Algo": One_percent_10min.ALGO,
            "Balance": self.manager.MANAGER_ALGO_RUN[One_percent_10min.ALGO][ex],
            "Target": target
        }
        ...
        # 재개
        self._run = True
        print("One 10 스레드 재가동")
        Manager.LOCK.release()
    # 개별시장마다
    ## 일단 업비트만
    def algo_onepercent_10min(self, money):
        time.sleep(1)
        for i in range(len(self.run_market)):
            ...
            try:
                ...
                if current_price >= self.target[market]:
                    ...
                    add_data(sell_order_id, One_percent_10min.DATAROAD)
                    print("DB 에 데이터 저장")
