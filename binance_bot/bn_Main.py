...
def initializer():
    bn_client.print_put("현재 시각 오전 9시@@@@")
    on_time = datetime.datetime.now(timezone('Asia/Seoul')).strftime('%Y-%m-%d %H:%M:%S %Z%z')
    bn_client.print_put(str(on_time))

    bn_client.print_put("현재 주문 내역 : " + str(bn_client.total_ordered_uid))
    ...
    bn_client.print_put("-----전일 주문 취소 완료!!-----")
    ...
    bn_client.print_put("금일 현재 My account Balance : " + str(bn_client.my_rest_balance) + " USDT")
    bn_client.print_put("-----금일 계좌 데이터 초기화 완료!!!-----")
    ...
    bn_client.print_put("btc 최적파라미터 : " + str(parameter_btc))
    parameter_eth = william_param(prev_eth_data, Bn_Client.BN_FEE)
    bn_client.print_put("eth 최적파라미터 : " + str(parameter_eth))
    parameter_bnb = william_param(prev_bnb_data, Bn_Client.BN_FEE)
    bn_client.print_put("bnb 최적파라미터 : " + str(parameter_bnb))

    bn_client.print_put("----- 파라미터 수정 완료!! -----")
